...
import com.s3m.formation.api.kpi.client.projection.TotalGrowthByMonthProjection;
...
""", nativeQuery = true)
    List<Object[]> getTotalGrowthByMonthForEntreprise(@Param("entrepriseId") Integer entrepriseId);
...
daily_totals AS (
    SELECT jour, formation,
           SUM(session_total_hours) AS total_heures
    FROM session_hours
...
ORDER BY y.annee
""", nativeQuery = true)
    List<Object[]> getTotalGrowthByYearForEntreprise(@Param("entrepriseId") Integer entrepriseId);
